...
# quat:     [num_positive, 4]
# proj_ctr: [num_positive, 2]  offseted x y in input image plane
# inv_intrinsics: [num_positive, 3, 3]
def allocentric_to_egocentric(quat, proj_ctr, inv_intrinsics):
    ...
    """
    # [num_positive, 3, 3]   rotate matrix
    R_obj_to_local = quaternion_to_matrix(quat)

    # ray == z-axis in local orientaion
    ray = unproject_points2d(proj_ctr, inv_intrinsics)   # [num_positive, 3]
...
def unproject_points2d(points2d, inv_K, scale=1.0):
    ...
    Tensor:
        ...
        E.g., (N, 3) or (N, K, 3) or (N, H, W, 3)
    """
    points2d = homogenize_points(points2d)  # [num_positive, 3]  +  1 
    siz = points2d.size()
    points2d = points2d.view(-1, 3).unsqueeze(-1)  # (num_positive, 3, 1)
