...
# a res-block contains 2 conv layers
...
# concat input in channel  ->  conv to out_channels(s = 1) -> (if residual = True, add input[0]) -> relu
...
class Tree(nn.Module):
    def __init__(
        ...
        if levels == 1:
            ...
            # level root = false, stride=1, root_dim= 3 * out + in
            self.tree2 = Tree(
                levels - 1,
                block,
                out_channels,
        ...
        if levels == 1:
            # Root: concat input in channel  ->  conv to out_channels(s = 1) -> (if residual = True, add input[0]) -> relu
...
class DLA(Backbone):
    ...
    #    residual_root=False,   # False
    #    # return_levels=False, 
    #    out_features=None,     # [level3, level4, level5]
    #    norm="BN",             # FrozenBN
    #):
    def __init__(
        self, cfg, levels, channels, block
    ):
        """
        ...
        """
        args = self.build_from_config(cfg, levels, channels, block)
        levels = args['levels']
        ...
        self.base_layer = Conv2d(     # conv 3 -> 16
            3,
            channels[0],
            kernel_size=7,
            stride=1,
    ...
    @classmethod
    def build_from_config(cls, cfg, levels, channels, block):
...
def build_fcos_dla_fpn_backbone_p67(cfg, input_shape):
    ...
    Returns:
        backbone (Backbone): backbone module, must be a subclass of :class:`Backbone`.
    """
    # channels:  {level3:128, level4:256, level5:512}
    # downratio: {level3:8,   level4:16,  level5:32}
    ...
    in_channels_p6p7 = out_channels  # 256
    # {'p3':tensor, 'p4':tensor, 'p5':tensor, 'p6':tensor, 'p7':tensor}
    backbone = FPN(
        bottom_up=bottom_up,
        in_features=in_features,     # [level3, level4, level5]
        out_channels=out_channels,   # 256
        norm=cfg.FE.FPN.NORM,        # FrozenBN
        # aditional 2 conv layers,  [out_channels(s=2), out_channels(s=2*2)]
        top_block=LastLevelP6P7(in_channels_p6p7, out_channels, "p5"),
        fuse_type=cfg.FE.FPN.FUSE_TYPE,   # sum
    )
