...
def get_label_anno(label_path):
    ...
    return annotations
    #name,truncated,occluded,alpha,bbox(4),dimensions(l,h,w),location(3)
    #rotation_y,score,index(0,1,2,...numobject-1),group_ids(0,1,2,...numobject-1)
...
def get_label_annos(label_folder, image_ids=None, return_ids = False):
    ...
    for idx in image_ids:
        ...
        anno["image_idx"] = np.array([idx] * num_example)
        annos.append(anno)
    if return_ids:
    ...
    #name,truncated,occluded,alpha,bbox(4),dimensions(l,h,w),location(3)
    #rotation_y,score,index(0,1,2,...numobject-1),group_ids(0,1,2,...numobject-1)
    #image_idx(img_index,repeat numobject times)
