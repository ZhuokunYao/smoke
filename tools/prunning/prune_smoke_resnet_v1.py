...
def arg_parse():
    ...
    parser.add_argument("--save_dir", type = str,
        default = "pretrained/pruned/")
    parser.add_argument('--percent', type=float, help='the ratio of prunning',
        default=0.5)
    parser.add_argument('--header', type=bool, help='Whether to prune the header layers',
                        default=False)
...
remained_id = [3, 4, 12, 13]

skip_layer_id = [28, 29, 44, 45, 60, 61]

stage_layer_id = [18, 19, 34, 35, 50, 51, 66, 67]
...
def calc_total_bn(model, features):
    ...
    for i, id in enumerate(features):
        ...
        if isinstance(layer, nn.Conv2d):
            if id in remained_id or id in skip_layer_id  or id in stage_layer_id:
                continue
    ...
    for index, id in enumerate(head_features):
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            if name in predictor_layer:
                ...
                ch, mask = calc_pruned_mask_for_layer(modules, head_features, index, thre, pruned=args.header)
                pruned = pruned + (mask.shape[0] - ch)
                head_channel.append(ch)
...
if __name__ == '__main__':
    ...
    pruned_weight = (args.ckpt.split('/')[-1])[:-4] + "_p{}_v1.pth".format(int(args.percent*10))
